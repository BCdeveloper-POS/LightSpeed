...
namespace LightspeedRetail_Api
{
    public class clsLightspeedRetail_Api
    {
        ...
        public async Task RunAsync()
        {
            try
            {
                await LightspeedRetail_ApiAsync(StoreId,tax, BaseUrl, ClientId, ClientSecret, RefreshToken,AccountID,IsMarkUpPrice, MarkUpValue);                
            }
            catch (Exception ex)
            {
        ...
        public async Task LightspeedRetail_ApiAsync(int storeid, decimal tax, string BaseUrl, string ClientId, string ClientSecret, string RefreshToken, int AccountID, bool IsMarkUpPrice, int MarkUpValue)
        {
        ...
        public async Task<List<JArray>> LightspeedSetting(string BaseUrl, int StoreId, decimal tax, string ClientId, string ClientSecret, string RefreshToken, int AccountID)
        {
            ...
            var accesstoken = await getProduct(BaseUrl, StoreId, ClientId, ClientSecret, RefreshToken);
...
*/
            ...
            {
                Console.WriteLine(ex.Message + " LightspeedRetailAPI ");
                (new clsEmail()).sendEmail(DeveloperId, "", "", "Error in " + " Lightspeed Pos@" + DateTime.UtcNow + " GMT", " ERROR  In Response while getting Product Pagination " + ":" + ex.Message + "<br/>" + ex.StackTrace);
            }
            return productList;
        }
